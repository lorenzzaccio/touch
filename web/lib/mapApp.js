...
var svgEle;
function mapApp(lclSvgEle,adjustVBonWindowResize,resizeCallbackFunction) {
        svgEle = lclSvgEle;
	...
	if (!svgEle.getScreenCTM) {
		...
		//this part is only required for viewers not supporting svgEle.getScreenCTM() (e.g. ASV3)
...
//set viewBox of svgEle to innerWidth and innerHeight
...
mapApp.prototype.calcInvCoord = function(svgPoint) {
	if (!svgEle.getScreenCTM) {
		...
		var matrix = svgEle.getScreenCTM();
	}
...
ViewBox.prototype.getTM = function() {
    var svgRoot      = svgEle;
    var matrix       = svgEle.createSVGMatrix();
